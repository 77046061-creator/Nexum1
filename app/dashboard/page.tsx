    ...
    <div className="relative min-h-screen">
      ...
      <div className="pl-60 transition-all duration-300">
        <div className="mx-auto max-w-6xl px-6 pt-6 pb-28">
          <div className="group relative overflow-hidden rounded-2xl border border-primary-container/20 bg-gradient-to-r from-primary-container/10 via-[#0d1f3c]/80 to-secondary-container/10 p-5 shadow-sm backdrop-blur-xl transition-all hover:shadow-md">
            ...
            <div className="relative flex items-center gap-4">
              <div className="relative flex-1">
                <svg className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-blue-200/70" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                  <circle cx="11" cy="11" r="8" /><path d="M21 21l-4.35-4.35" />
                </svg>
                <input
                  type="text"
                  placeholder="Buscar archivos, materias..."
                  className="w-full rounded-xl border border-white/20 bg-[#0d1f3c]/80 py-2.5 pl-10 pr-4 text-body-md text-white/90 outline-none transition focus:border-primary-container focus:ring-2 focus:ring-primary-container/30"
                />
              </div>
              <div className="flex items-center gap-2">
                <a
                  href="/api/auth/logout"
                  className="hidden rounded-lg border border-white/20 px-3 py-1.5 text-label-sm text-blue-200/70 transition-all hover:border-error-container/50 hover:bg-error-container/10 hover:text-error sm:block"
                >
                  Salir
                </a>
          ...
          {activeTab === "dashboard" || activeTab === "stats" ? (
            ...
            {[
              ...
              { label: "Favoritos", value: favCount || 0, icon: "M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z", gradient: "from-pink-500/20 via-[#0d1f3c]/60 to-rose-500/20", iconBg: "bg-pink-500/20 text-pink-300", ring: "ring-pink-500/30" },
            ].map((stat) => (
              <div
                key={stat.label}
                className="group relative overflow-hidden rounded-2xl border border-white/40 bg-[#0d1f3c]/80 p-4 shadow-sm backdrop-blur-sm transition-all hover:-translate-y-1 hover:shadow-lg"
              >
                ...
                <div className="relative">
                  ...
                  <p className="mt-0.5 text-label-sm text-blue-200/70">{stat.label}</p>
                </div>
              </div>
            ))}
          </div>
              ...
              <div className="mt-4 overflow-hidden rounded-2xl border border-white/30 bg-[#0d1f3c]/70 shadow-sm backdrop-blur-sm">
                {recentItems?.map((item, i) => (
                  <Link
                    key={item.id}
                    href={`/item/${item.id}`}
                    className={`group flex items-center gap-4 px-5 py-3.5 transition-colors hover:bg-gradient-to-r hover:from-primary-container/10 hover:via-transparent hover:to-transparent ${
                      i < (recentItems.length - 1) ? "border-b border-white/10" : ""
                    }`}
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-body-md font-medium text-white/90 group-hover:text-white transition-colors truncate">
                        {item.title}
                      </p>
                      <div className="mt-0.5 flex items-center gap-2 text-label-sm text-blue-200/70">
                        {item.subjects?.display_name && (
                          <span className="inline-flex items-center gap-1 rounded-full bg-primary-fixed-dim/20 px-2 py-0.5 text-white">
                            {item.subjects.display_name}
                          </span>
                        )}
                    ...
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="shrink-0 text-blue-200/30 transition-colors group-hover:text-primary-container">
                      <path d="M9 5l7 7-7 7" />
                    </svg>
                  </Link>
                ))}
            ...
            <div className="mt-8 grid gap-6 lg:grid-cols-2">
              {recentItems && recentItems.length > 0 && (
                <div className="rounded-2xl border border-white/30 bg-gradient-to-br from-[#0d1f3c]/80 via-[#0d1f3c]/60 to-primary-container/10 p-5 shadow-sm backdrop-blur-sm">
                  <div className="flex items-center gap-2">
                    <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-primary/10 text-xs text-white">⚡</div>
                    <h3 className="text-label-md font-semibold text-white/90">Actividad reciente</h3>
                  </div>
                  <div className="mt-4 space-y-3">
                    {recentItems.map((item, i) => (
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-body-sm font-medium text-white/90 group-hover:text-white transition-colors truncate">
                            {item.title}
                          </p>
                          <p className="text-label-xs text-blue-200/70">
                            {i === 0 ? "Justo ahora" : `Hace ${i + 1} min`}
                          </p>
                        </div>
                        <span className="flex h-6 w-6 items-center justify-center rounded-full bg-primary-fixed-dim/20 text-xs text-white">{i + 1}</span>
                      </Link>
                    ))}
                  </div>
                </div>
              ...
              {popularDocs && popularDocs.length > 0 && (
                <div className="rounded-2xl border border-white/30 bg-gradient-to-br from-[#0d1f3c]/80 via-[#0d1f3c]/60 to-secondary-container/10 p-5 shadow-sm backdrop-blur-sm">
                  <div className="flex items-center gap-2">
                    <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-secondary/10 text-xs text-secondary">🔥</div>
                    <h3 className="text-label-md font-semibold text-white/90">Más descargados</h3>
                  </div>
                  <div className="mt-4 space-y-3">
                    {popularDocs.map((item) => (
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-body-sm font-medium text-white/90 group-hover:text-secondary transition-colors truncate">
                            {item.title}
                          </p>
                          <p className="text-label-xs text-blue-200/70">
            ...
            <div className="mt-8">
              ...
              <div className="mt-4 grid gap-4 sm:grid-cols-2">
                {topSubjectsList.map(([name, count], i) => {
                  const colors = [
                    { bar: "bg-gradient-to-r from-primary-container to-primary", chip: "bg-primary/10 text-white" },
                  ...
                  return (
                    <div key={name} className="group rounded-2xl border border-white/30 bg-[#0d1f3c]/70 p-4 shadow-sm backdrop-blur-sm transition-all hover:shadow-md">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <div className={`flex h-8 w-8 items-center justify-center rounded-lg ${c.chip} text-xs font-semibold`}>
                            {name.charAt(0)}
                          </div>
                          <p className="text-body-md font-medium text-white/90">{name}</p>
                        </div>
                        <span className="text-label-sm font-semibold text-blue-200/70">{count}</span>
                      </div>
                      ...
                      <p className="mt-1.5 text-label-xs text-blue-200/70">{count} archivo{count !== 1 && "s"}</p>
                    </div>
                  );
                })}
              </div>
              ...
              <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {communityItems.slice(0, 6).map((item) => (
                  <Link
                    key={item.id}
                    href={`/item/${item.id}`}
                    className="group relative overflow-hidden rounded-2xl border border-white/30 bg-gradient-to-br from-[#0d1f3c]/80 to-tertiary-container/10 p-5 shadow-sm backdrop-blur-sm transition-all hover:-translate-y-1 hover:shadow-lg"
                  >
                    ...
                    <div className="relative">
                      <div className="flex items-center gap-2">
                        <span className="rounded-full bg-white/10 px-2.5 py-0.5 text-label-sm text-blue-200/70">
                          {typeLabels[item.type] || item.type}
                        </span>
                        {item.is_verified && (
                          <span className="flex items-center gap-1 rounded-full bg-primary-fixed-dim/30 px-2 py-0.5 text-label-sm text-white">
                            ✓ Verificado
                          </span>
                        )}
                      </div>
                      <h3 className="mt-2 text-body-md font-semibold text-white transition-colors group-hover:text-primary-container">
                        {item.title}
                      </h3>
                      <div className="mt-3 flex flex-wrap items-center gap-2 text-label-sm text-blue-200/70">
                        {item.subjects?.display_name && (
                          <span className="rounded-full bg-primary-fixed-dim/20 px-2 py-0.5 text-white">
                            {item.subjects.display_name}
                          </span>
                        )}
                        {item.users?.username && (
          ...
          {activeTab === "dashboard" && myCount === 0 && (
            <div className="mt-20 flex flex-col items-center text-center">
              ...
              <p className="mt-2 max-w-md text-body-md text-blue-200/70">
                ...
                <code className="rounded-md bg-white/20 px-2 py-0.5 text-label-sm font-mono text-white">
                  /subir
                </code>
              </p>
              <div className="mt-8 grid grid-cols-3 gap-4 text-center">
                {[
                  ...
                  <div key={t.label} className="rounded-2xl bg-[#0d1f3c]/60 p-4 shadow-sm backdrop-blur-sm ring-1 ring-white/50">
                    <span className="text-2xl">{t.icon}</span>
                    <p className="mt-1 text-label-sm text-blue-200/70">{t.label}</p>
                  </div>
                ))}
              </div>
            </div>
              ...
              <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {items?.map((item) => (
                  ...
                  >
                    ...
                    <div className="relative">
                      <div className="flex items-center gap-2">
                        <span className="rounded-full bg-gradient-to-r from-surface-container to-primary-fixed-dim/20 px-2.5 py-0.5 text-label-sm text-blue-200/70">
                          {typeLabels[item.type] || item.type}
                        </span>
                        {item.is_verified && (
                          <span className="flex items-center gap-1 rounded-full bg-primary-fixed-dim/30 px-2 py-0.5 text-label-sm text-white">
                      ...
                      {item.subjects && (
                        <p className="mt-1 text-label-sm text-blue-200/70">
                          {item.subjects.display_name}
                        </p>
                      )}
                      <div className="mt-3 flex items-center gap-3 text-label-sm text-blue-200/70">
              ...
              <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {communityItems.slice(0, 6).map((item) => (
                  <Link
                    key={item.id}
                    href={`/item/${item.id}`}
                    className="group relative overflow-hidden rounded-2xl border border-white/30 bg-gradient-to-br from-[#0d1f3c]/70 to-[#0d1f3c]/40 p-5 shadow-sm backdrop-blur-sm transition-all hover:-translate-y-1 hover:shadow-lg"
                  >
                    <div className="pointer-events-none absolute -inset-px rounded-2xl bg-gradient-to-br from-primary-container/8 via-transparent to-transparent opacity-0 transition-opacity group-hover:opacity-100" />
                    <div className="relative">
                      <div className="flex items-center gap-2">
                      ...
                      <div className="mt-2 flex items-center gap-2 text-label-sm text-blue-200/70">
                        {item.subjects?.display_name && (
                          <span className="rounded-full bg-primary-fixed-dim/20 px-2 py-0.5 text-white">
                            {item.subjects.display_name}
                          </span>
